...
## Mapping to the `tds-forge` quality gate

The `quality`/`compile` gate statically enforces a subset of this catalog on `.prw`/`.tlpp` sources. **Security (G1) blocks; legacy/quality (G2/G3/G4) warns.** Findings are tagged with the rule id (e.g. `[CA2050]`). Run with `--gate-warn-only` to downgrade every blocking rule to a warning.

| Rule(s) | Gate severity | Notes |
|---|---|---|
| CA2022, CA2023, CA2053 | BLOCK | StaticCall / PTInternal / CREATE PROCEDURE |
| CA2024, CA2025 (+ cFilAnt, cNumEmp) | BLOCK | tenant/user context assignment (legacy rule, pre-existing) |
| CA2050/CA2051 | BLOCK | only a **bare variable** in a SQL value; framework calls (`xFilial`, `RetSqlName`, …) excluded |
| CA2052 | BLOCK | credential var assigned a string literal (URLs/paths excluded) |
| BG1000 | BLOCK | `RpcSetEnv`/`RpcSetType` only when the source looks like REST/SOAP |
| CA1000, CA1003, CA1004 | WARN | ISAM drivers / API-in-loop / console output |
| CA2000–CA2012 | WARN | direct `DbSelectArea`/`DbUseArea` on system tables |
| CA1005, CA3001, CA4000 | WARN | .ini refs / include casing / inline IIf (rolled up per file) |
| TDS-DELET, TDS-FILIAL, TDS-TCQUERY, TDS-RECLOCK | WARN | authorial heuristics derived from common-error references |

Not statically enforced (require semantic analysis or are too noisy): CA2016 (i18n), CS1000 (any raw query), CA1001/CA2014–CA2021 (deprecated/binary APIs). Use SonarQube itself for full coverage.
